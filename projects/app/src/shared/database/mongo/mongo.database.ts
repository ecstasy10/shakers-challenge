...
export class MongoCollection<T extends Document> implements ICollection<T> {
    ...
    async find(filter: Partial<T> = {}): Promise<T[]> {
        ...
        results = await this.collection.find(filter as any).toArray();
    ...
    async findOne(filter: Partial<T>): Promise<T | null> {
        ...
        result = await this.collection.findOne(filter as any);
        if (!result) {
            return null;
        }
    ...
    async delete(filter: Partial<T>): Promise<number> {
        ...
        result = await this.collection.deleteMany(filter as any);
        if (!result) {
            return 0;
        }
